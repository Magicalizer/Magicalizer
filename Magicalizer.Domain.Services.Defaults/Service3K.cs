...
namespace Magicalizer.Domain.Services.Defaults
{
    ...
    where TFilter : class, IFilter
  {
    ...
    {
      return this.EntityToModel(await this.GetRepository().GetByIdAsync(id1, id2, id3, this.ToInclusions(inclusions)));
    }
    ...
    public virtual async Task<bool> DeleteAsync(TKey1 id1, TKey2 id2, TKey3 id3)
    {
      try
      {
        this.GetRepository().Delete(id1, id2, id3);
        await this.storage.SaveAsync();
        return true;
      }
    ...
    protected virtual IRepository<TKey1, TKey2, TKey3, TEntity, TFilter> GetRepository()
    {
      return this.storage.GetRepository<TKey1, TKey2, TKey3, TEntity, TFilter>();
    }
  }
}