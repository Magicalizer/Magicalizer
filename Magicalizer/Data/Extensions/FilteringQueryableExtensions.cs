    ...
    foreach (PropertyInfo property in properties)
    {
      ...
      else if (IsEnumerableFilter(property))
      {
        ...
        if (enumerableFilter.Any != null)
        {
          ...
          {
            Expression? enumerableFilterExpression = BuildEnumerableFilterExpression(parameter, propertyPath, property, anyFilter, isNone: false);

            if (enumerableFilterExpression != null)
              filterExpression = CombineExpressions(filterExpression, enumerableFilterExpression);
          }
        }
        ...
        if (enumerableFilter.None != null)
        {
          ...
          {
            Expression? enumerableFilterExpression = BuildEnumerableFilterExpression(parameter, propertyPath, property, noneFilter, isNone: true);
...
  private static Expression? BuildEnumerableFilterExpression(ParameterExpression parameter, IList<string> propertyPath, PropertyInfo property, IFilter filter, bool isNone)
  {
    ...
    Expression anyExpression = Expression.Call(enumerableAnyMethod.MakeGenericMethod(entityType), enumerableExpression, filterLambda);

    return isNone ? Expression.Not(anyExpression) : anyExpression;
  }
